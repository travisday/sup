...
struct SignUpView: View {
    ...
    private func signUp() {
        ...
        if(!self.viewModel.username.isAlphanumeric) {
            self.errorMessage = "Must use only letters and numbers for username"
            return
        }
        
        UserSearchService.searchUsernames(input: self.viewModel.username.lowercased()) { users in
            if !users.isEmpty {
